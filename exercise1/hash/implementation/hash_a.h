...
*** for subexercise a) you should only optimize performance                  ***
*** change only insert and findPos (don't change the element order)          ***
*******************************************************************************/
...
public:
    explicit HashA(size_t size) : table(size * 1.3) {}
...
private:
    ...
    IteratorType makeIterator(size_t pos) { return IteratorType(&table[pos], &table.back() + 1); }

    ConstIteratorType makeCIterator(size_t pos) const { return ConstIteratorType(&table[pos], &table.back() + 1); }
